...
```{r}
mat_melt %>%
  dplyr::filter(value >= 4.5) %>%
  dplyr::filter(Var1 != "White" & Var2 != "White") %>%
...
mat_melt %>%
  dplyr::filter(value >= 4.5) %>%
  dplyr::filter(Var1 != "White" & Var2 != "White") %>%
  dplyr::filter(Var1 == "Cast Iron" | Var2 == "Cast Iron") %>%
  dplyr::arrange(desc(value))
```
...
col_names <- c("Duke Navy Blue", "Ginger Beer", "Duke Royal Blue", "Magnolia", "Ironweed", "Dandelion", "Cast Iron", "Piedmont")

mat_melt %>%
  dplyr::filter(Var1 %in% col_names & Var2 %in% col_names) %>%
  dplyr::arrange(desc(value))
```
