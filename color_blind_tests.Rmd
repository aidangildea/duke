...
```{r}
# Final Option #1
final_vals1 <- c("#012169","#A1B70D", "#00539B", "#339898", "#C84E00", "#988675",  "#E89923", "#262626")
final_names1 <- c("Duke Navy Blue", "Piedmont", "Duke Royal Blue", "Eno", "Copper", "Dogwood", "Persimmon", "Cast Iron")

final_melt <- mat_melt %>%
  dplyr::filter(Var1 %in% final_names1 & Var2 %in% final_names1)


 ggp_final <- ggplot(final_melt1, aes(Var1, Var2, fill = value)) +      
# Create heatmap with ggplot2
  geom_tile(#aes(fill = value)
    ) +
  labs(title = "Color-Contrast Ratios Between Color Pairings", x = "Color 1", 
...
final_melt %>%
  filter(value >= 3) %>% 
  dplyr::filter(Var1 == "Duke Navy Blue" | Var2 == "Duke Navy Blue") %>%
  arrange(desc(value)) %>%
