...
#library(colorratio)
#remotes::install_github("matt-dray/coloratio", force = TRUE)
```
...
## Dummy Data Set
To better investigate which color palette is preferable, we will create a dummy data set that has an 8-level categorical variable. This allows us to see how compatible and accessible colors are when placed on the same plot. 
...
```{r, warning = FALSE}
# testing different potential palettes 
...
# all available colors in the Duke color guide
...
# displays color palettes by different color vision deficiencies
...
s2 <- edit_colors(splot +
                    scale_duke_color_discrete(), deutan, sev = 0.7)
cowplot::plot_grid(splot +
                     scale_duke_color_discrete(), s2)
```
...
mat_melt <-  reshape2::melt(ratioMatrix)
```

```{r heatmap, fig.asp=.618}
png(file="~/Desktop/dukethesis/images/contrastheatmap.png")
...
```{r}
# Final Option #1
final_vals <- c("#012169","#A1B70D", "#C84E00", "#00539B", "#339898", "#E89923", "#FFD960", "#262626")

# old vals
#c("#012169","#A1B70D", "#00539B", "#339898", "#C84E00", "#993399",
                     #  "#262626", "#E89923")

final_names <- c("Duke Navy Blue", "Piedmont", "Copper", "Duke Royal Blue", "Eno", "Persimmon", "Dandelion", "Cast Iron")
...
ggplot(penguins, aes(bill_length_mm, bill_depth_mm, color = species)) +
  geom_point() +
  scale_color_manual(values = final_vals1)

ggplot(penguins, aes(bill_length_mm, bill_depth_mm, color = species)) +
  geom_point() +
  scale_color_manual(values = final_vals2)
...
dumby_df %>%
  ...
  labs(title = "Grade Distribution") +
  theme_duke() +
  scale_color_manual(values =  final_vals1,  aesthetics = "fill")
...
dumby_df %>%
  ...
  labs(title = "Grade Distribution") +
  theme_duke() +
  scale_color_manual(values =  final_vals2,  aesthetics = "fill")
```

```{r}
ratings <-final_melt %>%
  #filter(value >= 3) %>% 
  dplyr::filter(Var1 %in% final_names | Var2 %in% final_names, value != 1) %>%
  arrange(desc(value)) %>%
  distinct(value, .keep_all = T)


colnames(ratings) <- c("Color 1", "Color 2", "Contrast Ratio")

saveRDS(ratings, "~/Desktop/dukethesis/saved/contrastratios.rds.gzip", 
        compress = "gzip")
...
final_melt %>%
  filter(Var1 %in% c("Duke Navy Blue", "Piedmont", "Copper", "Duke Royal Blue", "Eno"), Var2 %in% c("Duke Navy Blue", "Piedmont", "Copper", "Duke Royal Blue", "Eno")) %>%
  arrange(desc(value)) %>%
