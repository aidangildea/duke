test_that("using theme_duke produces desired stylistic changes", {
  ...
  p <- p + theme_duke()
  expect_true(p$theme$plot.title$family %in% c("Garamond 3 LT Std", "sans"))

  # Adding theme_duke() makes the caption have Hyperlegible font
  p <- p + theme_duke()
  expect_true(p$theme$plot.caption$family %in% c("hyperlegible", "sans"))
...
test_that("adding theme object to ggplot object with + operator works", {
  ...
  p <- ggplot2::ggplot(data.frame(x = 1:3), ggplot2::aes(x, x)) + ggplot2::geom_point() + theme_duke()
  ...
  p <- ggplot2::ggplot(data.frame(x = 1:3), ggplot2::aes(x, x)) + ggplot2::geom_point()
  p <- p + ggplot2::theme(axis.title = ggplot2::element_text(size = 20))
  expect_true(p$theme$axis.title$size == 20)
